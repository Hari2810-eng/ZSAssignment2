...
public class DungeonGame2 {
	...
	public static void game(int adventureX, int adventureY, int goldX, int goldY, int monsterX, int monsterY, int m, int n){
        DungeonGame2 dungeon = new DungeonGame2(m, n);
        adventureX -= 1;
        adventureY -= 1;
        goldX -= 1;
        goldY -= 1;
        monsterX -=1;
        monsterY -=1;
        int adventureStep = dungeon.findMinSteps(adventureX, adventureY, goldX, goldY);
        int monsterStep = dungeon.findMinSteps(adventureX, adventureY, goldX, goldY);
        System.out.println(adventureStep+","+ monsterStep);
        if (adventureStep == -1) {
